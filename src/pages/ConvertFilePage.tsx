...
export default function ConvertFilePage() {
  ...
  useEffect(() => {
    if (convertToDS) {
      let sdata = JSON.parse(
        JSON.stringify(
          sheetData[currentSheet as keyof typeof sheetData] as unknown as [][]
        )
      );
  ...
  function generateConvertCommandObject(
    jsonData: TableRow[],
    tableName: string
  ): ConvertCommand {
    ...
    const createTableQuery = `CREATE TABLE IF NOT EXISTS ${newTblName} (${columns
      .map(
        (col) =>
    ...
    const insertTableQuery = `INSERT INTO ${tableName.replace(
      /[^a-zA-Z0-9]/g,
      "_"
    )} (${SQLcolumns.join(", ")}) VALUES ${insertValues};`;
      ...
      DatabaseService.postDatabase(dbname, userId)
        .then((res) => {
          ...
          visibleSheetNames.map((sheet, i) => {
            let sheetSD = JSON.parse(
              JSON.stringify(
                sheetData[sheet as keyof typeof sheetData] as unknown as [][]
              )
            );
            ...
            let uniquetblName =
              sheet.replace(/[^a-zA-Z0-9]/g, "_") + "_" + uid();
            ...
            sql2dArr.push(
              generateConvertCommandObject(dataSrc as TableRow[], uniquetblName)
            );
          });
          setSQLCmds(sql2dArr);
        })
